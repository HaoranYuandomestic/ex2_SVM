...
\begin{document}
	...
	\section*{一、实验内容}
	这次实验我做的是支持向量机在多分类任务上的实现。按照题目要求，我没有直接调用现成的 SVM 分类函数，而是自己写了一个线性二分类 SVM，再在这个基础上扩展成多分类模型。具体来说，我分别实现了二分类 SVM、One-vs-Rest 和 One-vs-One 两种多分类策略，然后在人工生成的三分类数据集上做训练和测试。
	...
	\]
	前一项控制模型复杂度，后一项约束分类间隔。训练时，我采用了逐样本更新的方式来近似做梯度下降。
	
	因为实验是三分类任务，所以我又在二分类 SVM 的基础上扩展了多分类策略：
	\begin{itemize}
	...
	\section*{六、核心代码}
	下面给出本次实验中最关键的一部分代码：核函数定义、支持向量机实现和多分类策略。
...
class OneVsOneSVM:
	...
	\begin{enumerate}
		...
		\item 核函数计算的效率问题。 对于高斯核等复杂核函数，计算两两之间的核值需要大量计算。我通过使用 NumPy 的向量化操作来加速计算。在决策函数计算时，使用 $f(x) = \sum_{i=1}^m \alpha_i y_i \kappa(x, x_i) + b$ 形式，避免显式特征映射。
	...
	\section*{十、实验总结}
	这次实验让我更深入地理解了支持向量机的精妙之处。特别是核函数的引入，使我认识到：
	\begin{enumerate}
